...
public final class Utils {
	...
	byte paddingByte) throws java.io.UnsupportedEncodingException {
		int txtByteLen = text.getBytes(characterSetName).length;
//		if( txtByteLen >= length) {
...
//		}
		...
		switch( alignment) {
			...
			case ALIGN_LEFT:
				System.arraycopy( text.getBytes( characterSetName), 0, byte_array, 0, txtByteLen);
				break;

			case ALIGN_RIGHT:
				int t_offset = length - text.length();
				System.arraycopy( text.getBytes( characterSetName), 0, byte_array, t_offset, txtByteLen);
				break;
			}	
